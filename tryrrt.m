classdef tryrrt
    ...
    methods
        ...
        function plot(obj)
            figure;
            hold on;
            plot([0 0 100 100 0], [0 100 100 0 0],'c-');%cyan
            %fill([35 25 25 75 75 65 65 35 35], [35 35 65 65 35 35 55 55 35],'c');
            plot([45 45 55 55 45], [55 75 75 55 55],'c-');%cyan
            plot([45 45 55 55 45], [25 45 45 25 25],'c-');%cyan
            for i=1:size(obj.Ta,1)
                if i==1
                    plot(obj.Ta(1,1),obj.Ta(1,2),'or');
                else
                    index = obj.Ta(i,3);
                    plot([obj.Ta(i,1) obj.Ta(index,1)],[obj.Ta(i,2),obj.Ta(index,2)], 'b-');
                    plot(obj.Ta(i,1),obj.Ta(i,2), 'k.', 'MarkerSize', 8);
                end
            end
            
            for i=1:size(obj.Tb,1)
                if i==1
                    plot(obj.Tb(1,1),obj.Tb(1,2),'ob');
                else
                    index = obj.Tb(i,3);
                    plot([obj.Tb(i,1) obj.Tb(index,1)],[obj.Tb(i,2),obj.Tb(index,2)], 'r-');
                    plot(obj.Tb(i,1),obj.Tb(i,2), 'k.', 'MarkerSize', 8);
                end
            end
        ...
        function Xnew = updateXsoln(obj)
            if abs(obj.Ta(1,1)-obj.start(1,1))<0.0001 && abs(obj.Ta(1,2)-obj.start(1,2))<0.0001
                X = obj.ellipseNodesA;
            else
                X= obj.ellipseNodesB;
            end
        ...
        function [len,path] = caculateOptimizationPath(obj,A,B,soln)
            totalPath = obj.extract_path(A(1:soln(1,1),:), B(1:soln(1,2),:));
            mm=1;
            nn=3;
            path = totalPath(mm,:);
        ...
        function final_path = extract_path(~,treeA, treeB)
